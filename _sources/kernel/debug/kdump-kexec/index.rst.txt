...
- SIGSEGV: 非法内存访问，比如访问空指针或者越界数组

- SIGBUS: 总线错误，通常是硬件相关的内存访问问题，比如对齐错误

- SIGABRT: 程序主动调用abort函数中止，比如assert失败

- SIGFPE: 浮点运算异常，比如除零

- SIGALL: 非法指令，可能代码被篡改或者编译器生成的问题

- SIGQUIT: 用户通过键盘触发的退出信号

- SIGSYS: 非法系统调用，可能调用了内核不支持的API

- SIGTRAP: 断点或跟踪陷阱，通常是调试相关
